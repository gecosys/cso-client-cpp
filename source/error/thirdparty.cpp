...
std::string ThirdParty::getOpensslError(int32_t code) {
    ...
    return UtilsString::format("[OpenSSL]: %s", reason);
}
...
std::string ThirdParty::getCurlError(int32_t code) {
    ...
    return UtilsString::format("[CURL]: %s", reason);
}
...
std::string ThirdParty::getSocketError(int32_t code) {
    ...
    return  UtilsString::format("[Socket]: %s", reason);
#elif defined(_MSC_VER) // MSVC
    char* reason = nullptr;
    FormatMessageW(
    ...
    auto result = UtilsString::format("[Socket]: %s", reason);
    LocalFree(reason);
    return result;
#endif
}